...
## 2026-06-12 - Recomputing Derived Lists in BlocBuilder
**Learning:** Mapping and sorting lists (an O(N log N) operation) directly inside a `BlocBuilder`'s `build` method creates a performance bottleneck that re-executes on every frame if unoptimized.
**Action:** Memoize derived lists within a `StatefulWidget` using `identical(state.list, _lastList)` to ensure recomputation only happens when the underlying identity changes, and change `elementAt(index)` on sets or iterables to index access on cached lists.
