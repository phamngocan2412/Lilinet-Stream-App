...
## 2026-04-01 - Memoizing Derived Lists in BlocBuilder
**Learning:** Recomputing derived lists or sets (e.g., dynamically mapping and sorting `FavoritesPage`'s folder list, an O(N log N) operation) directly inside a `BlocBuilder`'s build method executes on every frame, causing performance bottlenecks.
